...
function getPageDeets() {
	...
	function getProdPrice() {
		var price = parseFloat($("[data-test='product-price']").text().replace(/[$,\s]/g, ''));
		if (!price) {
			price = parseFloat($('.details-pdp .price').text().replace(/[$,\s]/g, ''));
		}
		console.log('product price div');
		console.log($("[data-test='product-price']"));
		console.log($('.details-pdp .price'));
		price = price ? price : '';
		return price;
	}
	...
	function getProductImage() {
		...
		if (!imageSrc) {
			imageSrc = $('.slideDeckPicture img').attr('src');
		}
