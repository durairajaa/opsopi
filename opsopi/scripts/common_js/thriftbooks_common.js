function back_search_tk(prod_deets) {
	...
	dyn_req.done(function(response, status, xhr) {
		var extracted_deets = extract_result(response, status, xhr);
	...
	function extract_result(response) {
		...
		if (resp_elem_wm.find('.Search-results-container').length > 0) {
			...
			try {
				deets['prod_link'] = searchURL;
				deets['title'] = $.trim($(firstResult).find('.SearchResultGridItem-title').text());
				deets['prod_price'] = parseFloat(
					$(firstResult).find('.SearchResultGridItem-price').text().split('-')[0].replace(/[\s$,]/g, '')
				);
				deets['website'] = 'tk';
			...
			try {
				...
				// 	deets['prod_link'] =
				// 		'https://www.thriftbooks.com/w' + deets['prod_link'].split('www.thriftbooks.com')[1];
				// }
