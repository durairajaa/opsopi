!(function(e) {
	...
	(t.prototype.stripComments = function(e) {
		...
		(t.prototype.parseCSS = function(e) {
			...
			for (var r, i = new RegExp(this.cssKeyframeRegex, 'gi'); null !== (r = i.exec(e)); )
				t.push({
					selector: '@keyframes',
					type: 'keyframes',
					styles: r[0]
			...
			for (var n = new RegExp(this.combinedCSSRegex, 'gi'); null !== (r = n.exec(e)); ) {
				...
				var l = new RegExp(this.cssCommentsRegex, 'gi'),
					p = l.exec(o);
				if (
					(null !== p && (o = o.replace(l, '').trim()), -1 !== (o = o.replace(/\n+/, '\n')).indexOf('@media'))
				) {
					var a = {
						selector: o,
						type: 'media',
		...
		(t.prototype.findBySelector = function(e, t, s) {
			...
			for (var r = [], i = 0; i < e.length; i++)
				!1 === s ? e[i].selector === t && r.push(e[i]) : -1 !== e[i].selector.indexOf(t) && r.push(e[i]);
			...
			for (var s = '', r = 0; r < e.length; r++)
				void 0 !== e[r] &&
					(void 0 === e[r].defective
						? (s += this.getSpaces(t) + e[r].directive + ': ' + e[r].value + ';\n')
				...
				if (
					...
					if ('media' !== n.type) {
						for (var o = n.selector.split(','), l = [], p = 0; p < o.length; p++)
							-1 === o[p].indexOf('.supernova') ? l.push(r + ' ' + o[p]) : l.push(o[p]);
						n.selector = l.join(',');
				...
				if (imag_class_list.indexOf('sc-cFlXAS') > -1) {
					...
					if (imag_class_list.length > 0) {
						...
						var class_rule = parsed_css.filter(function(css_rule) {
							if (
								css_rule.selector == class_with_bg_img &&
								css_rule.hasOwnProperty('rules') &&
								is_property_in_rule_list('background-image', css_rule.rules)
							) {
								return true;
							}
						});
