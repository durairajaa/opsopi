...
function fetch_site_for_user_site(deets, title_to_search) {
	var request_url = make_search_url_from_pattern(
		deets.search_url_pattern,
		deets.url_space_delimiter,
		title_to_search
	);
	...
	user_site_fetch_req.done(function(response, b, c) {
		...
		if (div.querySelector(deets.first_elem_selector)) {
			...
			if (result_deets['title'] && result_deets['image'] && result_deets['price'] && result_deets['link']) {
				...
				deets_to_display['prod_link'] = make_valid_link_for_user_added_sites(
					result_deets['link'],
					deets.host_name
				);
				...
				deets_to_display['img_src'] = make_valid_link_for_user_added_sites(
					result_deets['image'],
					deets.host_name
				);
